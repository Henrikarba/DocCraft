import { SvelteParser } from './svelte/svelteParser';
import type { ComponentDoc } from './svelte/svelteParser';
...
export class DocGenerator {
	...
	async generateDocs(
	...
	): Promise<{ components: ComponentDoc[] }> {
		...
		if (isSingleFile) {
			...
			// Find all relevant files based on framework
			files = await this.findFiles(inputPath);
		}
		...
		for (const file of files) {
			try {
				...
				const outputFile = path.join(outputPath, `${componentDoc.name}.md`);
		...
		async function walk(directory: string) {
			...
			for (const entry of entries) {
				...
				if (entry.isDirectory() && !entry.name.startsWith('.')) {
					...
					// Add different file extensions based on framework
